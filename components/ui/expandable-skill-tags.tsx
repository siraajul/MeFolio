  ...
  return (
    <section className={cn("w-full flex flex-col items-center text-center", className)}>
      ...
      >
        ...
        {visibleSkills.map((skill, index) => (
          <motion.div key={`visible-${index}`} variants={itemVariants}>
            <Badge variant="secondary" className="text-sm py-1.5 px-3 bg-muted/50 hover:bg-brand/20 hover:text-brand transition-colors cursor-default">{skill}</Badge>
          </motion.div>
        ))}
      ...
      {skills.length > initialCount && (
        <Button
          variant="link"
          className="mt-4 px-0 text-sm text-muted-foreground hover:text-brand"
