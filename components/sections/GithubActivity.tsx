...
export function GithubActivity({ username = "siraajul" }: GitHubActivityProps) {
  ...
  useEffect(() => {
      ...
      try {
        const response = await fetch(
          `https://github-contributions-api.jogruber.de/v4/${encodeURIComponent(username)}?y=last`,
          {
            ...
            // Ensure credentials are not sent to third-party API
            credentials: "omit",
          }
        );
        ...
        if (json.contributions && Array.isArray(json.contributions)) {
          // Validate each contribution entry
    ...
    // Cleanup function
    return () => {
      // Abort controller handled in finally block
    };
  }, [username]);
  ...
  if (error || data.length === 0) {
    return (
      <section className="w-full flex flex-col items-center justify-center py-12 px-4">
        <div className="w-full max-w-5xl p-8 rounded-3xl border border-border/50 bg-card/30 backdrop-blur-sm shadow-xl text-center">
